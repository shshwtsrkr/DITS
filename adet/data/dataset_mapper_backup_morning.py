...
class DatasetMapperWithBasis(DatasetMapper):
    ...
    def __init__(self, cfg, is_train=True):
        ...
        self.enhanced_images = set()
        with PathManager.open("adet/data/enhanced_images.txt", "r") as f:
            for line in f:
                self.enhanced_images.add(line.strip())
    ...
    def __call__(self, dataset_dict):
        ...
        try:
            ...
            if dataset_dict["file_name"] in enhanced_images:
                enhancement_factor = 1.5  # Update the annotations and other parameters by a factor of 1.5
                ...
                for anno in dataset_dict["annotations"]:
                    ...
                    # x1, y1, x2, y2 = [coord * enhancement_factor for coord in anno["bbox"]]
                    # x1, x2 = min(max(x1, 0), w-1), min(max(x2, 0), w-1)
                    # y1, y2 = min(max(y1, 0), h-1), min(max(y2, 0), h-1)
                    # anno["bbox"] = [x1, y1, x2, y2]
                    # print(f"BBOX for image {image}: ",x1,y1,x2,y2)
